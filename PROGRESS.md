...
## 現在のフェーズ: ✅ T7 旧コード整理・ドキュメント（完了）→ リリース待ち

### v2 リアーキテクチャ フェーズ一覧

| フェーズ | 内容 | 状態 |
|---|---|---|
| T0 | 準備・ブランチ戦略 | ✅ 完了 (2026-05-07) |
| T1 | ドメイン知識ベース | ✅ 完了 (kanshi 60組・gengou 41件・kyuusei 9種・kyuujitai 71字) |
| T2 | エンジン抽象化レイヤー | ✅ 完了 (OCREngine Protocol・engines/base.py) |
| T3 | Claude エンジン全面リプレイス | ✅ 完了 (engines/claude_engine.py・prompts/) |
| T4 | パイプライン構築 | ✅ 完了 (extractor・validator・corrector・splitter・classifier) |
| T5 | ハイブリッドエンジン実装 | ✅ 完了 (engines/hybrid_engine.py・accuracy_eval) |
| T6 | UI 統合 | ✅ 完了 (モード選択・確定進捗・コスト推定・結果ボタン・バリデーション) |
| T7 | 旧コード整理・ドキュメント | ✅ 完了 (2026-05-07) |
| **リリース** | v2-dev → main マージ・v2.0.0 タグ | ⬜ 残 |
...
| フェーズ4 | CLI仕上げ | ✅ 完了 |
| フェーズ5 | UI（後日） | ✅ PyQt6 launcher.py 完成 |
| フェーズ6 | Claude APIエンジン | ✅ v2 で全面リプレイス済み |

---

## 🚨 現在の課題: Claude API エンジンの精度

### 状況
- Web版 Claude (claude.ai) に同じPDFをアップロードすると**ほぼ正確**に読める
- 本ツールの Claude API エンジン (`claude_engine.py`) では**精度が劣る**
- PaddleOCR・NDLOCR はさらに精度が低い

### 文書の特徴
- 江戸時代の古文書（寺社関係の記録・台帳）
- **縦書き**、右→左読み
- **元号**（正保・寛文・延宝・天和・貞享・元禄・宝永・享保など）
- **干支**（甲子・乙丑・丙寅など）
- **旧字体・異体字**（繁体字に近い字形）
- **見開きスキャン**（左右でわずかに傾きが異なる）
- 罫線で区切られた**表形式**（台帳・帳面）

### claude_engine.py の現在のプロンプト
```
この画像は日本語文書のスキャンです。画像内のすべてのテキストを正確に書き起こしてください。
- 縦書きは右→左の列順、横書きは上→下の行順で読む
- 元号・干支・旧字体・異体字はそのまま書き起こす（変換しない）
- 段落・列の区切りは改行で表現する
- 表・図・欄外の注記も含める
...
### 改善したいこと
1. **プロンプトの最適化** — web版と同等の精度を出すプロンプトを設計したい
2. **表構造の抽出精度** — 表のセル境界を正確に認識してExcelに出力したい
3. **出力フォーマット** — 列・行の構造を保ったまま書き起こしたい
...
### フェーズ5 — UI（後日）
- [x] UIフレームワークの選定 → PyQt6 を採用
- [x] `launcher.py` の基盤実装（ドロップゾーン・ファイルチップ・設定パネル・ログ・進捗）
- [x] ドラッグ＆ドロップ対応（PDF/PNG/JPG/TIFF）
- [x] エンジン選択（PaddleOCR / NDLOCR）
- [x] DPI・出力先設定
- [x] バックグラウンドスレッド（OCRWorker）でノンブロッキング実行
- [x] ダークモード自動対応（システムパレット検出）
- [ ] 処理履歴パネル
- [ ] 詳細設定（ページ範囲・言語）
- [ ] 検索可能PDF出力オプション
...
| 2026-05-07 | Claude（ベイダー） | v2 リアーキテクチャ T0〜T7 全フェーズ実装完了。knowledge/・engines/・pipeline/・exporters/ に整理。249テスト全通過。README v2 書き換え |
